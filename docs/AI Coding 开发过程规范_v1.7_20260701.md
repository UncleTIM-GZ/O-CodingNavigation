...
版本：1.7
发布日期：2026-07-01
基于工具版本：O'CodingNavigator 0.7.0（`o-coding-navigation@0.7.0-beta.6`）· SOP profile 0.7.0（自 DEC-039 起包版本与 SOP 版本统一并锁步：SOP profile 直接升至 0.7.0 与 npm 对齐，内容等价于 0.5.0；旧版 profile 仍冻结可导入。就绪门准时激活、默认接线与人类推进短标志均为引擎/CLI 特性）
...
本文件与 OCN 工具版本的对应关系如下：本规范 1.7 对应 SOP profile 0.7.0，发行载体为 npm 包 `o-coding-navigation@0.7.0-beta.6`（含 `ocn` 命令行工具与 `ocn-mcp` MCP 服务器）。自 DEC-039 起 npm 包版本与 SOP profile 版本统一并今后锁步推进。本文件中引用的命令、退出码、检查项与文件路径均以该版本的实际行为为准；工具版本升级时，本文件应同步修订。
...
AI 辅助软件开发过程中存在一类系统性缺陷，本文件统称为虚假完成（false completion）：过程产物在形式上满足完成条件，而其所代表的工程事实并未成立。按判定对象的不同，虚假完成分为五类（其中第五类的判定为可选，须由项目显式启用，见 5.5）：
...
本文件适用于使用 OCN 0.7.0 及兼容版本组织 AI 辅助软件开发的项目，包括单人项目、小团队项目与平台级项目（分级要求见 5.4.3）。
...
- `o-coding-navigation@0.7.0-beta.0`（npm 发行包，含 `ocn` 命令行工具与 `ocn-mcp` MCP 服务器）
...
过程产物在形式上满足完成条件而其所代表的工程事实未成立的缺陷，分五类，见 0.1。
...
3. 就绪门禁——按 5.4 执行就绪检查；
4. 契约门禁（可选，仅启用时）——按 5.5 在 BUILD/VERIFY 执行漂移校验，运行于就绪门禁之后。

`ocn check`、`ocn gate`、`ocn advance` 三处的门禁组成与顺序应保持一致（含契约门禁），以避免判定面漂移。任务规格校验（5.3.4）属于 11 号文档的产物校验，归入第 2 类判定（退出码 2）；契约声明块校验（5.5.3）同属第 2 类判定。
...
| 2 | 产物缺失或无效 | ERR_ARTIFACT_INVALID | 必需章节缺失；逻辑主干命中六类硬缺陷；任务规格命中六类硬缺陷；契约声明块缺失或结构无效 |
...
```bash
npm install -g o-coding-navigation     # latest 通道，当前对应 SOP profile 0.7.0
ocn --version                          # 验证安装；预期输出 0.7.0-beta.0
...
ocn sop upgrade                        # 执行：迁移到当前内置默认版本（0.7.0）
```
