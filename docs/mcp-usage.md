...
从 v0.5.0-beta.0 开始，npm latest 与 beta 均指向 SOP 0.5.0 的 Plan → Build → Verify 闭环版本（含 Task Backbone（把守 BUILD 出口的冻结验收任务台账）、Readiness Backbone（基于角色的就绪门禁）与 Logic Backbone 特性）。如果希望明确固定在 beta 预发布通道，可以使用 @beta：
