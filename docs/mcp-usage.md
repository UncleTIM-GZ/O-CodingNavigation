...
As of v0.4.0-beta.0, npm latest and beta both point to the SOP 0.4.0 Plan → Build → Verify release with the Readiness Backbone (role-based readiness gate) and Logic Backbone features. Use @beta when you want to pin the prerelease channel explicitly:
...
从 v0.4.0-beta.0 开始，npm latest 与 beta 均指向 SOP 0.4.0 的 Plan → Build → Verify 闭环版本（含 Readiness Backbone（基于角色的就绪门禁）与 Logic Backbone 特性）。如果希望明确固定在 beta 预发布通道，可以使用 @beta：
