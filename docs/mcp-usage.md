...
As of v0.3.0-beta.1, npm latest and beta both point to the SOP 0.3.0 Plan → Build → Verify release with the Logic Backbone feature. Use @beta when you want to pin the prerelease channel explicitly:
...
从 v0.3.0-beta.1 开始，npm latest 与 beta 均指向 SOP 0.3.0 的 Plan → Build → Verify 闭环版本（含 Logic Backbone 特性）。如果希望明确固定在 beta 预发布通道，可以使用 @beta：
