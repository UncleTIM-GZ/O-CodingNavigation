...
|---|---|---|---|---|---|
| [AM-009](./2026-06-13-auto-mode-amendment.md) | 2026-06-13 | Auto Mode (optional per-phase delegation of advance / task check / milestone rewind to the AI agent, DEC-034) | Accepted | None (additive; restates "advance is human-only" as human-**authorized** — manual default unchanged and now technically enforced) | `docs/06` (`ocn auto` + `--actor`/`--rationale` flags, MCP whitelist unchanged), `docs/05` §12.15 (`auto_mode_changed`), `.ocoding/config.yaml` (`automation:` block) + `automation-runtime.json`, `src/core/automation/*` |
| [AM-008](./2026-06-12-rewind-cycle-amendment.md) | 2026-06-12 | `ocn rewind` + `ocn cycle new` (controlled cursor rollback + round restart, DEC-033) | Accepted | None (additive — frozen §25 `ocn reset` untouched; `rewind` yields the name) | `docs/06` (two new human-only commands, MCP whitelist unchanged), `docs/05` §12.15 (`cursor_rewind`, `cycle_started`), `.ocoding/cycles/<n>-<ts>/` archive layout, `src/core/rewind/*`, `src/core/cycle/*` |
