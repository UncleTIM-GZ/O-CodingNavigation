...
> 版本：`0.5.0-beta.1` · 适用范围：tester / 内部使用 · 状态：beta，非 GA
...
当前版本：`0.5.0-beta.1`（npm `latest` 与 `beta` 同指此版）。仅在 Claude Desktop on Windows + WSL2 验证。Cursor / Cline 未验证。仍是 beta，不是 GA。
