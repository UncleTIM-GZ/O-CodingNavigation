...
版本：1.6
发布日期：2026-06-28
基于工具版本：O'CodingNavigator 0.7.0（`o-coding-navigation@0.7.0-beta.0`）· SOP profile 0.5.0（自动模式、触发前独立评审与可选契约主干均为引擎/CLI 特性，包版本与 SOP 版本自 1.4 起解耦）
...
本文件与 OCN 工具版本的对应关系如下：本规范 1.6 对应 SOP 0.5.0，发行载体为 npm 包 `o-coding-navigation@0.7.0-beta.0`（含 `ocn` 命令行工具与 `ocn-mcp` MCP 服务器）。本文件中引用的命令、退出码、检查项与文件路径均以该版本的实际行为为准；工具版本升级时，本文件应同步修订。
