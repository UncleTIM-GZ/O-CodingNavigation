...
As of v0.4.0-beta.0, npm latest and beta both point to the SOP 0.4.0 Plan → Build → Verify release with the Readiness Backbone (role-based readiness gate) and Logic Backbone features.
...
Current npm dist-tags: `latest = 0.4.0-beta.0`, `beta = 0.4.0-beta.0`, `alpha = 0.1.0-alpha.2` (historical; preserved).
...
| `ocn --version` does not show `0.4.0-beta.0` | Installed an older cached version. | Re-install: `npm install -g o-coding-navigation@latest`, or pin the prerelease explicitly with `npm install -g o-coding-navigation@beta`. |
...
从 v0.4.0-beta.0 开始，npm latest 与 beta 均指向 SOP 0.4.0 的 Plan → Build → Verify 闭环版本（含 Readiness Backbone（基于角色的就绪门禁）与 Logic Backbone 特性）。
...
当前 npm dist-tag：`latest = 0.4.0-beta.0`、`beta = 0.4.0-beta.0`、`alpha = 0.1.0-alpha.2`（历史保留）。
...
| `ocn --version` 不是 `0.4.0-beta.0` | 安装到了旧的缓存版本。 | 重新安装：`npm install -g o-coding-navigation@latest`，或显式固定预发布通道：`npm install -g o-coding-navigation@beta`。 |
