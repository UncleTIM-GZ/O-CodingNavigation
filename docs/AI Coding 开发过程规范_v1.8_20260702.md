...
版本：1.8
发布日期：2026-07-02
基于工具版本：O'CodingNavigator 0.8.0（`o-coding-navigation@0.8.0-beta.0`）· SOP profile 0.8.0（自 DEC-039 起包版本与 SOP 版本统一并锁步；SOP profile 0.8.0 在既有能力上新增验收主干（验收规格门），npm 与 SOP 版本对齐为 0.8.0-beta.0；旧版 profile 仍冻结可导入。验收主干为 SOP profile 特性；就绪门准时激活、默认接线与人类推进短标志均为引擎/CLI 特性）
...
本文件与 OCN 工具版本的对应关系如下：本规范 1.8 对应 SOP profile 0.8.0，发行载体为 npm 包 `o-coding-navigation@0.8.0-beta.0`（含 `ocn` 命令行工具与 `ocn-mcp` MCP 服务器）。自 DEC-039 起 npm 包版本与 SOP profile 版本统一并今后锁步推进。本文件中引用的命令、退出码、检查项与文件路径均以该版本的实际行为为准；工具版本升级时，本文件应同步修订。
...
AI 辅助软件开发过程中存在一类系统性缺陷，本文件统称为虚假完成（false completion）：过程产物在形式上满足完成条件，而其所代表的工程事实并未成立。按判定对象的不同，虚假完成分为六类（其中第五类的判定为可选，须由项目显式启用，见 5.5；其余五类为强制判定）：
...
- **第五类（契约漂移型）**：设计阶段声明了接口契约，实施阶段写出的前端去调用该接口，但无任何环节校验二者一致。结构完整、逻辑接线、角色就绪、实现也确有发生，前端调用面却与声明的接口契约背离——例如前端调用了契约未声明的端点，或对同一路径使用了与契约不符的方法。该类判定为可选项，须由项目显式启用（见 5.5）。
- **第六类（验收非机器可读型）**：验收标准以散文或 Markdown 表格书写，未登记为机器可解析的验收规格块，导致其无法被任务规格的 traces 绑定、追溯链在起点断裂。表面看验收已定义，实则从未成为可绑定、可追溯的靶子；隐藏于表格中的验收标准既不能约束实现漂移，也不能支撑"某项实现对应哪条验收"的追溯。该类由 SPEC 阶段的验收门判定（见 5.6），为强制判定。
...
| 前端调用与声明契约一致吗 | 前端调用点是否均能对应到声明契约中的端点（方法与路径一致） | 契约门禁（声明契约与前端扫描交叉校验，可选） |
| 验收标准可绑定、可追溯吗 | 验收标准是否登记为机器可解析的验收规格块（编号合法、desc 非空、无重复） | 验收门禁（验收规格结构校验，冻结为 AC 编号权威源） |
...
本文件适用于使用 OCN 0.8.0 及兼容版本组织 AI 辅助软件开发的项目，包括单人项目、小团队项目与平台级项目（分级要求见 5.4.3）。
...
- `o-coding-navigation@0.8.0-beta.0`（npm 发行包，含 `ocn` 命令行工具与 `ocn-mcp` MCP 服务器）
- OCN 文档模板体系 `docs/00` 至 `docs/19`（随 SOP 0.8.0 分发的产物模板与必需章节定义）
...
过程产物在形式上满足完成条件而其所代表的工程事实未成立的缺陷，分六类，见 0.1。
...
对应某一验收角色关注点的可证伪检查项，判定结果为 PASS、FAIL、UNKNOWN、WAIVED、NA 或 DEFERRED（输入尚未到期，见 5.4.1）。
...
应将目标转换为可判定的验收项，使"已实现"成为可判断命题，为第二阶段的证据映射（`ocn evidence map`）提供基线，并满足 5.6 规定的验收规格要求。
...
**准时激活（不提前且不缺失）**：横切不等于一进场即全员生效。每条 block 级就绪检查应在其依赖输入到期的那一步起方才生效，到期点取该检查全部输入在 SOP 全局步序中**最晚**的产出步（文档类输入取产出该文档的步；仓库探针类输入取其策略状态的首步）。在到期步之前该检查判定为 DEFERRED（输入尚未到期，不阻断），自该到期步起逐步持续强制（不提前、且不可被跨过——带未满足的 block 检查不得到达 VERIFY）。该判定应为步级而非状态级：同一状态内、依赖较晚步产物的检查（如依赖 PRD 的检查）在更早步（如 scope 步）仍应判 DEFERRED，不得提前索要下一步的产物。任一依赖无法解析其到期步时，该检查不延迟、自首道门禁起强制（失效安全：宁可早判不漏判）。该机制为随 SOP profile 打包的开关项（`precise_activation`），自 SOP profile 0.7.0 起启用；未启用的旧版 profile 行为不变。

注：就绪机制自 OCN 0.4.0 起产品化为 readiness 横切门禁，准时激活自 0.7.0（AM-014 / DEC-040）起启用。设计参考：readiness review、Definition of Done、生产就绪评审（PRR）、按依赖图准时激活的适应度函数。
...
- **准时激活判定**：尚未到期的 block 级检查应判 DEFERRED（见 5.4.1），不阻断；DEFERRED 既非 FAIL 亦非 UNKNOWN，不参与阻断集，也不接受豁免。该判定应前瞻可见：在 `ocn brief` 列为"Forthcoming｜将到期（not due until <步>）"、在 `ocn readiness list` 标注 `[DEFERRED]` 与到期步，不静默隐藏。
- **warn 级检查**仅进入 `ocn brief` 提示，不阻断（例如战略层的过度准备检查）。
- **判定台账**：每轮评估应写入 `.ocoding/readiness.json`；`ocn brief` 应将未决项及其 fix_hint 列为工作清单，并单列将到期（DEFERRED）项。
...
契约主干为可选的横切门禁，默认关闭。其判定对象为第五类虚假完成（契约漂移型，见 0.1）。项目应在 `.ocoding/config.yaml` 的 `contract` 块显式启用后该门禁方生效：

- `contract.enabled`：布尔值，默认 `false`；
- `contract.frontendRoot`：前端源码根目录（默认 `src`）；
- `contract.basePath`（可选）：接口公共前缀，用于路径归一化。

未启用时，本节要求不适用，且 `ocn doc create api-contract` 等命令的输出与未启用时逐字节一致。

注：本机制自 OCN 0.7.0-beta.0 起产品化为 contract 门禁（对应 AM-012 / DEC-037）。设计参考：契约一致性测试（Pact）、OpenAPI 一致性校验、架构适应度函数。

### 5.5.2 声明即唯一事实源

启用后，设计阶段的接口契约产物（`docs/06-api-contract.md`，号随版本可变）应携带一个信息串恰为 `ocn-api-contract` 的代码栅栏，其内容为 YAML，声明全部公开端点：

```
endpoints:
  - id: endpoint_list_users        # 稳定字符串 id（§4.1 命名约定）
    method: GET
    path: /api/users               # :param 段为通配
```

栅栏标签即选择器：未打 `ocn-api-contract` 标签的 ` ```yaml ` / ` ```json ` 块一律不被采纳，以避免误将无关块当作契约。引擎不应从散文中臆造端点；声明的端点集即权威集。

### 5.5.3 声明块校验（设计阶段，退出码 2）

启用后，在接口契约产物所在步骤的产物校验中，应对声明块作结构校验。命中下列任一缺陷即返回 `ERR_ARTIFACT_INVALID`（退出码 2）：声明块缺失、声明块非有效 YAML、重复端点 id、重复 `(method, path)`、非法 HTTP 方法、路径缺少前导斜杠。已启用而声明块缺失属此类——启用即声明该块必须存在。

### 5.5.4 漂移校验（第二阶段，退出码 1）

启用后，契约门禁应在 BUILD 与 VERIFY 状态运行：经 TypeScript 编译器 API 抽取 `frontendRoot` 下 `.ts`/`.tsx` 的调用点，与声明端点交叉校验。仅高置信度（方法与路径在 `basePath` 归一化后均为静态字面量、且属于受限调用形式白名单）的违规参与阻断；命中未声明调用或方法不符时返回 `ERR_GATE_FAILED`（退出码 1），阻断离开 BUILD 与离开 VERIFY 的推进。通过时将规范化结果写入 `.ocoding/contract-graph.json`，由 `ocn brief` 注入覆盖摘要（端点/调用/未声明/方法不符/未验证计数）。

### 5.5.5 失效安全（不得假阻断）

契约门禁应遵循失效安全方向，不得在无法证明漂移时阻断：

- `frontendRoot` 不存在或 TypeScript 不可用时——无调用点可据以证明漂移——应跳过而非阻断；
- 只读调用方（MCP `run_gate`，不执行扫描）应报告"未验证"（`contract_unverified`，与就绪门禁的 UNKNOWN 同向取保守阻断），而非以未扫描状态判定为通过。
...
自 SOP 0.8.0 起，03 验收标准（`docs/03-acceptance-criteria.md`）应包含 `## Acceptance Specs｜验收规格` 章节，将验收标准拆分为机器可解析的验收规格块；每条验收标准应构成一个可绑定的最小规格块。其判定对象为第六类虚假完成（验收非机器可读型，见 0.1）。以散文或 Markdown 表格书写而未登记为验收规格块的验收标准不被采纳，不构成合规的验收标准定义（见 5.6.3）。

### 5.6.2 块格式

每个验收规格块应以 `### AC-<域>-<编号>` 为标题，字段格式如下例（示例为说明性内容）：

```markdown
### AC-CORE-01
- desc: 空目录执行 ocn init 后 state.json 指向 step_project_brief
- priority: P0
- given: 一个空目录
- when: 执行 ocn init --tier minimal
- then: state.json 生成且 currentStepId = step_project_brief
- trace: PRD-1.2
```
...
|---|---|---|
| desc | 必填 | 验收标准的单句陈述 |
| priority | 可选 | 优先级标记（如 P0、P1） |
| given | 可选 | 前置条件 |
| when | 可选 | 触发动作 |
| then | 可选 | 预期结果 |
| trace | 可选 | 追溯至上游需求（如 PRD 条目）的引用 |

### 5.6.3 四类硬缺陷判定

验收门应在 `step_acceptance_criteria` 步骤的产物校验中，于章节门禁之后运行，对验收规格判定下列四类硬缺陷，命中任一项即返回 `ERR_ARTIFACT_INVALID`（退出码 2）：

1. no_specs（无 `## Acceptance Specs` 章节，或章节存在但无任何 AC 块）；
2. duplicate_id（AC 编号重复）；
3. invalid_id（AC 编号不符 `AC-<域>-<编号>` 格式）；
4. missing_field（某 AC 块缺 `desc` 必填字段）。

判定应由构造保证：Markdown 表格或散文中的验收标准不被视为定义（空块即判 no_specs，缺 desc 即判 missing_field）；引擎不应以启发式判读表格内容。

### 5.6.4 机器投影与 traces 绑定

验收门通过时，引擎应将规范化的验收规格冻结写入 `.ocoding/acceptance-specs.json`；该文件为 AC 编号的权威机器源。build plan 任务规格的 traces 字段（5.3.3）应绑定至该文件所登记的 AC 编号——即验收主干产出 AC 编号，任务主干的 traces 引用之。

读取应具备陈旧性保护：03 验收标准自门禁通过后未再修改时，后续读取使用冻结投影；03 一经修改，则改用实时结构化解析。因此删除某条 AC 后，仍以 traces 引用该 AC 的任务应被判定为 traces 悬空（5.3.4 第 3 项），不应因投影而误判为通过。

注：本机制自 OCN 0.8.0 起产品化为 acceptance 产物门（对应 AM-015 / DEC-041）。运行时默认 profile 随之切至 0.8.0；0.7.0 及更早 profile 冻结可导入，`ocn sop upgrade` 迁移（新必需章节落于已通过的 SPEC 步）。MCP 白名单维持 7 项工具不变。设计参考：Given/When/Then 验收准则、契约测试的可追溯矩阵。
...
`ocn check` 应按下列顺序执行门禁：

1. 章节门禁——校验必需章节的存在与非空；
2. 验收规格门禁——校验 5.6.3 所列四类硬缺陷（03 号文档就位后，运行于 SPEC 阶段该步的章节门禁之后）；
3. 逻辑主干门禁——校验 5.2.4 所列六类硬缺陷（07 号文档就位后）；
4. 就绪门禁——按 5.4 执行就绪检查；
5. 契约门禁（可选，仅启用时）——按 5.5 在 BUILD/VERIFY 执行漂移校验，运行于就绪门禁之后。

`ocn check`、`ocn gate`、`ocn advance` 三处的门禁组成与顺序应保持一致（含契约门禁），以避免判定面漂移。验收规格校验（5.6.3）属于 03 号文档的产物校验、任务规格校验（5.3.4）属于 11 号文档的产物校验，二者均归入第 2 类判定（退出码 2）；契约声明块校验（5.5.3）同属第 2 类判定。
...
|---|---|---|---|
| 0 | 通过 | OK | 门禁通过；命令正常完成 |
| 1 | 门禁未通过 | ERR_GATE_FAILED | 就绪检查存在 FAIL 或 UNKNOWN 的必需项；契约门禁检出漂移（未声明调用 / 方法不符） |
| 2 | 产物缺失或无效 | ERR_ARTIFACT_INVALID | 必需章节缺失；验收规格命中四类硬缺陷（no_specs/duplicate_id/invalid_id/missing_field）；逻辑主干命中六类硬缺陷；任务规格命中六类硬缺陷；契约声明块缺失或结构无效 |
...
- 启用契约门禁的项目，于 BUILD 与 VERIFY 状态检出契约漂移（5.5.4）时，`advance` 不应放行；处置路径为修正前端调用面使其落入声明契约，或在设计阶段补充/修正声明端点后重过门禁。`frontendRoot` 缺失或 TypeScript 不可用时该门禁跳过，不阻断推进（5.5.5）。
...
自 AM-013 / DEC-038 起，`ocn init` 默认执行下述接线（等价于自动调用一次 `ocn agent setup`）；以 `ocn init --no-agent` 可退出，仅写 `.ocoding/` 而不接线。接线为失效安全：接线未完成不影响 init 成功，仅提示可手动补跑 `ocn agent setup --force`。
...
由于上述注入，操作者在已接线的代理会话内亲自执行手动专属操作时，命令亦携带 `OCN_ACTOR=ai_agent` 而被拒绝。操作者应以人类身份覆盖：手动模式下推进应执行 `ocn advance --human`（`-H`，等价 `--actor user` 并覆盖环境签名），或在未接线的普通终端直接执行 `ocn advance`。手动专属操作的拒绝提示应给出该可照抄的命令。
...
| NC-11 | 验收非机器可读：验收标准写在散文或 Markdown 表格中，未登记为验收规格块，任务规格 traces 无从绑定，追溯链断裂 | 将验收标准写入 03 号文档的 `## Acceptance Specs` 章节，每条验收一个 `### AC-<域>-<编号>` 块、`desc` 必填（5.6）；验收门通过后冻结为 `.ocoding/acceptance-specs.json`，作为 AC 编号权威源供 build plan traces 绑定。表格中的验收标准不视为定义（5.6.3）；无对应机器块的 traces 应判为悬空（5.3.4） |
...
```bash
npm install -g o-coding-navigation     # latest 通道，当前对应 SOP profile 0.8.0
ocn --version                          # 验证安装；预期输出 0.8.0-beta.0
...
ocn sop upgrade                        # 执行：迁移到当前内置默认版本（0.8.0）
```
...
```bash
cd <项目目录>
ocn init --tier minimal                # 创建 .ocoding/ 与 docs/，锁定 SOP 0.8.0
...
| 1.5 | 2026-06-15 | 新增自动模式触发前的独立评审要求（对应 AM-011 / DEC-036）：自动模式下无人工逐步把关，AI 编码代理在被授权阶段内行使任一受控触发（`advance` / `task check` / 里程碑回拨）前，应先委派独立、全新上下文的评审子代理以资深专家视角校验实现，给出 PASS/FAIL；该评审为替代被省略人工评审的尽职调查，裁定权仍属门禁与冻结验收命令。FAIL 时记录—修正—复审，上限 3 次，仍不过则写入 `--rationale` 后继续；结论仅覆盖当次改动。新增术语 3.28（独立评审）；新增 7.6.8（触发前的独立评审），原 7.6.8 治理文案顺延为 7.6.9 并补充自动循环区块前置评审步骤；职责划分 8.1 增列触发前评审；附录 A.7 同步。引擎/CLI 文本层特性，不 bump SOP 版本；MCP 白名单 7 工具不变 |
| 1.6 | 2026-06-28 | 新增可选契约主干（Contract Backbone，对应 OCN 0.7.0-beta.0 / AM-012 / DEC-037）：判定第五类虚假完成（契约漂移型）。设计阶段接口契约产物可携带 `ocn-api-contract` 声明块作为唯一事实源；启用后，声明块结构缺陷在设计阶段判第 2 类（退出码 2），BUILD/VERIFY 经 TypeScript 编译器 API 抽取前端 `.ts`/`.tsx` 调用点与声明端点交叉校验，高置信度漂移判第 1 类（退出码 1）。失效安全：前端根缺失或 TypeScript 不可用时跳过、只读调用报"未验证"而不假通过。新增第五类虚假完成（0.1）及方法表对应行（0.2）；新增 5.5（契约要求，含启用、声明、声明块校验、漂移校验、失效安全五小节）；7.1 门禁组成增列契约门禁并声明 check/gate/advance 三处同序，7.2 退出码补充契约情形，7.3 推进规则增列契约阻断条款。默认关闭，未启用时一切输出逐字节不变；引擎/CLI 特性，不 bump SOP 版本；MCP 白名单 7 工具不变 |
| 1.7（本规范） | 2026-07-01 | 三项引擎/CLI 修订，均不改变功能性技术要求的实质，仅细化时机、统一版本号与补充工具便利。①**就绪门准时激活**（对应 OCN 0.7.0-beta.3/.6 / AM-014 / DEC-040）：block 级就绪检查改为自其依赖输入到期的那一步起方才生效（到期点取全部输入在 SOP 全局**步序**中最晚的产出步），到期前判 DEFERRED（不阻断、前瞻可见），自到期步起逐步持续强制——不提前（消除 minimal 档第一步即被全部下游检查阻断的悬崖）、且不缺失（带未满足 block 检查不得到达 VERIFY）；步级而非状态级（依赖 PRD 的检查在 scope 步仍 DEFERRED）；依赖不可解析则不延迟（失效安全）。修订 3.16（判定增列 DEFERRED）、5.4.1（增"准时激活"条款）、5.4.4（增 DEFERRED 判定与前瞻可见要求）。开关 `precise_activation` 随 SOP profile 0.7.0 启用；旧版 profile 行为不变。②**npm 包版本与 SOP profile 版本统一并锁步**（对应 DEC-039）：SOP profile 直接升至 0.7.0 与 npm 对齐（跳过 0.6.0），内容等价于 0.5.0；旧版 profile 冻结可导入；修订版本对应关系（前言、5.4.1 注、附录 A 升级与默认版本）。③**`ocn init` 默认接线 Claude Code**（对应 AM-013 / DEC-038）+ **`ocn advance --human`/`-H`**：init 默认执行 agent setup（`--no-agent` 退出、失效安全）；因接线注入 `OCN_ACTOR=ai_agent`，操作者在代理会话内手动推进应以 `ocn advance --human` 覆盖。修订 7.5、7.6.4。引擎/CLI 特性，不 bump SOP 工作流语义；MCP 白名单 7 工具不变 |
| 1.8（本规范） | 2026-07-02 | 新增验收主干（Acceptance Backbone，对应 OCN 0.8.0 / AM-015 / DEC-041）：判定第六类虚假完成（验收非机器可读型）。SPEC 阶段的 03 验收标准应含 `## Acceptance Specs｜验收规格` 章节，一条验收标准一个 `### AC-<域>-<编号>` 块（`desc` 必填，`priority`/`given`/`when`/`then`/`trace` 可选）；验收门在 `step_acceptance_criteria` 于章节门之后运行，硬拦四类结构缺陷（no_specs/duplicate_id/invalid_id/missing_field → 退出码 2），通过后将验收规格冻结为 `.ocoding/acceptance-specs.json`，作为 AC 编号权威机器源供 build plan 任务规格的 traces 绑定；读取带陈旧性保护（03 未改用冻结投影、改动即实时解析，故删除 AC 后仍 trace 它的任务被正确判为悬空）。新增第六类虚假完成（0.1）及方法表对应行（0.2）；术语 3.6 改为分六类；新增 5.6（验收规格要求，含书写位置、块格式、四类硬缺陷、机器投影与 traces 绑定四小节），5.1.4 增列满足 5.6；7.1 门禁组成增列验收规格门禁（03 就位后、章节门之后），7.2 退出码 2 补充验收规格情形；不符合项新增 NC-11。运行时默认 profile 切至 0.8.0（npm 与 SOP 版本锁步为 0.8.0-beta.0），0.7.0 及更早 profile 冻结可导入、`ocn sop upgrade` 迁移；SOP profile 特性，MCP 白名单 7 工具不变 |
