...
## 修订版 v15
## 基于 OCN 两阶段模型 + 四主干（逻辑 · 就绪 · 任务 · 验收）+ 验收主干（SOP 0.8.0）+ 可选契约主干 + Claude Code 默认接线 + 可选自动模式（含触发前独立评审）+ 就绪门准时激活

> 修订日期：2026-07-02 · 对应 `o-coding-navigation@0.8.0-beta.0`（SOP profile 0.8.0）
...
**前半段防失控，后半段防失真；四根主干（外加可选的契约主干），把"假完成"挡在每一道门外。**
...
## 1.9 验收标准要机器可读，traces 才有靶子

章节齐了、逻辑接线了、角色签收了、实施也真发生了，还剩一处能悄悄漏气的地方：验收标准本身。如果验收标准只写在散文或 Markdown 表格里，机器读不进去、也就无从登记——于是 build plan 里任务规格的 `traces` 找不到可绑定的靶子，门禁却可能照样绿灯。

- 验收标准要写成**机器可解析的块**（一条验收一个块），门禁硬拦结构缺陷，由构造保证，不靠启发式去猜表格
- 通过后**冻结成投影**，成为 AC 编号的权威机器源——任务主干的 `traces` 正是绑到这些 AC 上（验收主干产出靶子，任务主干瞄准它）
- 封堵的是"AC 藏在表格里的假追溯"：藏起来的验收既拦不住漂移，也撑不起可追溯

这就是**验收主干**：把"验收标准写清楚了吗"这个不可验证的命题，换成"每条验收都登记成可绑定的机器块了吗"这个可验证的命题。详见 3.8。
...
- **验收要机器可读、traces 才有靶；实现只认冻结验收命令的 exit 0。**——机械防线版：1.9（验收主干把 AC 登记成可绑定的机器靶）与 1.8（任务主干勾销只认冻结命令）合写，四根主干各守一类假完成。
- **不是让 Agent 一路写下去，而是让系统在关键节点知道现在发生了什么、下一步该做什么。**——AI Coding 版：1.5（AI 是执行放大器，不是责任替代者）加上 1.6–1.9 四根主干的机械防线，合成的交互节奏。
...
```bash
npm install -g o-coding-navigation     # latest 通道，当前即 SOP 0.8.0
ocn --version                          # 验证：0.8.0-beta.0
...
ocn sop upgrade                        # 执行：迁移到当前内置默认（0.8.0）
```
...
```bash
cd <你的项目>
ocn init --tier minimal                # 创建 .ocoding/ 与 docs/，锁定 SOP 0.8.0（并默认接线 Claude Code，--no-agent 退出）
...
#   （人 + AI 写内容）                  # ③ 填实内容
ocn check                              # ④ 三道门：章节 → 逻辑主干 → 就绪（03 另过验收规格门、11 另过任务规格门）
...
- **exit 2**（产物问题）：缺必需章节、**03 验收规格命中四类缺陷**（no_specs/duplicate_id/invalid_id/missing_field）、07 逻辑主干命中六类缺陷（缺角色/重复 id/悬空引用/环/孤儿/未绑定触发），或 **11 build plan 的任务规格命中六类缺陷**（重复/非法 id、缺字段、traces 悬空、touches 悬空、depends 悬空/成环、零任务）——回去补文档本身
...
- **回拨零豁免**——`rewind` 只移动游标，docs/ 一个字不动；之后每次 `advance` 都重过完整门禁（章节 + 验收 + 逻辑 + 就绪 + 任务）。它把项目送回裁判面前，不替你过关。`--reason` 必填，全文进审计。
...
#   exit 2 → 补章节/验收规格/逻辑主干/任务规格；exit 1 → ocn readiness list → 补证据或 waive
#   03 验收标准必含 Acceptance Specs 块；门禁通过即冻结验收规格投影（AC 编号权威源）
#   11 build plan 必含 Task Specs 拆分（traces 绑定 03 的 AC 编号）；门禁通过即冻结任务台账
...
### 验收主干（Acceptance Backbone，v15 新增）——03 的机器可解析部分

自 SOP 0.8.0 起，03 验收标准必须包含 `## Acceptance Specs｜验收规格` 章节：把验收标准**拆成机器可解析的块**，一条验收标准一个 `### AC-<域>-<编号>` 块，每个块是一份可绑定的最小验收项：

```markdown
### AC-CORE-01
- desc: 空目录执行 ocn init 后，.ocoding/state.json 存在且指向 step_project_brief
- priority: P0
- given: 一个空目录
...
（`desc` 必填；`priority` / `given` / `when` / `then` / `trace` 可选。）

**为什么要机器可解析**：验收标准若写在 Markdown 表格或散文里，机器**读不进去、也就静默不登记**——于是 build plan 里任务规格的 `traces` 无从绑定，门禁却可能照样绿灯。这是一类新的假完成——**“AC 藏在表格里的假追溯”**：验收看似写了，实则从未成为可追溯的靶子。验收主干由构造把它堵死：表格里的 AC 不算定义，空块即拦、traces 悬空即拦，引擎从不靠启发式去猜表格里哪几行是验收。

**四类硬缺陷**，任一命中即 03 验收标准门禁阻断（`ERR_ARTIFACT_INVALID`，退出码 2）：

1. `no_specs`——无 `## Acceptance Specs` 章节，或章节存在但零个 AC 块；
2. `duplicate_id`——AC 编号重复；
3. `invalid_id`——AC 编号不符 `AC-<域>-<编号>` 格式；
4. `missing_field`——某 AC 块缺 `desc` 必填字段。

**门禁在哪运行**：验收门在 `step_acceptance_criteria`（SPEC 阶段）于章节门之后运行——它是 03 这份产物自己的产物门，与逻辑主干门跑在 07、任务规格门跑在 11 同构。

**门禁通过的瞬间**：把规范化的验收规格冻结进 `.ocoding/acceptance-specs.json`——它成为 **AC 编号的权威机器源**。build plan 任务规格里的 `traces` 就绑定到这些 AC 编号上（把 3.12 任务主干的 `traces` 与本主干接上：**验收主干产出 AC 编号，任务主干的 traces 指向它**）。

**陈旧性保护**：03 自门禁通过后未再改动，则后续读取用冻结投影；一旦 03 被改动，则改走实时结构化解析。于是删掉一条 AC 之后，仍 `trace` 它的任务会被正确判为 traces 悬空（不会假绿）——投影不会盖住已删除的 AC。

在 OCN 中，这一原则自 0.8.0 起产品化为 `acceptance` 产物门。运行时默认 profile 随之切到 0.8.0（内容与就绪/契约等既有能力一致，npm 与 SOP 版本锁步 → npm `0.8.0-beta.0`）；0.7.0 及更早 profile 冻结且可导入，`ocn sop upgrade` 迁移（新必需章节落在已过的 SPEC 步）；MCP 白名单 7 工具不变。对应 AM-015 / DEC-041，详见 `docs/acceptance-backbone-proposal.md` 与 `docs/amendments/2026-07-02-acceptance-backbone-amendment.md`。范式参考 Given/When/Then 验收准则、契约测试的可追溯矩阵。
...
这是 v13 新增的**可选契约主干**，与四根始终在线的主干不同的是：它**默认关闭，按需启用**。四根主干（逻辑/就绪/任务/验收）始终在线；契约主干只在项目显式打开 `contract.enabled` 后生效。
...
此外，部分产物自带专属门禁，跑在该步的章节门之后：SPEC 阶段的 03 验收标准过**验收规格门**（v15 新增，四类缺陷 → exit 2，详见 3.8），DESIGN 阶段的 07 逻辑主干过逻辑门，PLAN 阶段的 11 build plan 过任务规格门。它们与就绪横切门共同构成第一阶段的门禁栈——验收规格门产出的 AC 编号，正是任务规格门里 `traces` 绑定的靶子。
...
## 7.11 错误十一：验收标准非机器可读（AC 藏在表格里，traces 无靶）

验收标准写进了 Markdown 表格或散文，看着齐全，机器却读不进去、静默不登记——于是 build plan 里任务规格的 `traces` 找不到可绑定的 AC，追溯链断在起点。更隐蔽的是门禁可能照样绿灯：藏起来的 AC 既拦不住实现漂移，也撑不起"这条实现对应哪条验收"的追溯。这是"AC 藏在表格里的假追溯"。

纠偏：

把验收标准写进 03 的 `## Acceptance Specs｜验收规格` 块（验收主干，3.8）——一条验收一个 `### AC-<域>-<编号>` 块、`desc` 必填。验收门在 SPEC 阶段硬拦四类结构缺陷（no_specs/duplicate_id/invalid_id/missing_field → exit 2），通过后把验收规格冻结成 `.ocoding/acceptance-specs.json`，成为 AC 编号的权威机器源；随后 build plan 的 `traces` 绑到这些 AC 上，追溯链才闭合。表格里的 AC 不算定义（空块即拦、traces 悬空即拦），规范由构造保证、不靠启发式去猜表格。
...
## 本次 v15 修订：补上"验收主干"——让验收标准可绑定、可追溯

契约主干把第五类假完成也堵上后，回看整条追溯链，暴露出一个一直被默认"写了就有"的薄弱环节：**验收标准本身没有机器可解析的结构**。任务主干的 `traces` 要绑定到真实存在的 AC，可 03 验收标准若把验收写在 Markdown 表格或散文里，机器读不进去、静默不登记——`traces` 找不到靶子，门禁却可能照样绿灯。这是一类新的假完成：**"AC 藏在表格里的假追溯"**——验收看似写了，却从未成为可绑定、可追溯的靶子。

v15 补上**验收主干（Acceptance Backbone）**（对应 OCN 0.8.0 / AM-015 / DEC-041），解法仍与前几根同构——把"验收标准写清楚了吗？"换成"每条验收都登记成可绑定的机器块了吗？"：03 验收标准必须含 `## Acceptance Specs｜验收规格` 章节，一条验收一个 `### AC-<域>-<编号>` 块（`desc` 必填，`priority`/`given`/`when`/`then`/`trace` 可选）；验收门在 SPEC 阶段（`step_acceptance_criteria`）于章节门之后硬拦四类结构缺陷（no_specs/duplicate_id/invalid_id/missing_field → exit 2），通过后把验收规格冻结成 `.ocoding/acceptance-specs.json`——AC 编号的权威机器源，build plan 任务规格的 `traces` 正绑定于此（验收主干产出靶子，任务主干瞄准它）。读取带陈旧性保护：03 未改用冻结投影、一旦改动即实时解析，故删掉一条 AC 后仍 trace 它的任务被正确判为悬空、不会假绿。规范由构造保证——表格里的 AC 不算定义，不靠启发式去猜表格。运行时默认 profile 切到 0.8.0（npm/SOP 锁步 → npm `0.8.0-beta.0`），0.7.0 及更早 profile 冻结可导入、`ocn sop upgrade` 迁移；SOP profile 特性，MCP 白名单 7 工具不变。详见 3.8 与第七章错误十一。
