...
> **Phase**: SOP 0.3.0 (Logic Backbone) mainline · **Status**: pre-GA beta · **Public**: on npm as `latest` and `@beta` → [`0.3.0-beta.1`](https://www.npmjs.com/package/o-coding-navigation) · GitHub pre-release: [`v0.3.0-beta.1`](https://github.com/UncleTIM-GZ/O-CodingNavigation/releases/tag/v0.3.0-beta.1)
...
| npm | `latest` → `0.3.0-beta.1`; `beta` → `0.3.0-beta.1`; `alpha` → `0.1.0-alpha.2` (historical; preserved) |
...
- **npm publish discipline**: SOP 0.3.0 mainline published as `o-coding-navigation@0.3.0-beta.1` under strict pre-publish checklists, `prepublishOnly` gate, and `files` allowlist. `latest` and `beta` both point to `0.3.0-beta.1`; `alpha` is preserved at `0.1.0-alpha.2` for historical use. Annotated git tag `v0.3.0-beta.1` and matching GitHub pre-release published.
...
As of v0.3.0-beta.1, npm latest and beta both point to the SOP 0.3.0 (Logic Backbone) release.

Verify:

```bash
ocn --version       # 0.3.0-beta.1
ocn --help
...
|---|---|---|---|
| `latest` (recommended) | `0.3.0-beta.1` | `latest` | SOP 0.3.0 (Logic Backbone) mainline. |
| Beta (explicit prerelease pin) | `0.3.0-beta.1` | `beta` | Same artifact as `latest`; use `@beta` when you want to pin the prerelease channel explicitly. |
...
- Current external package: `0.3.0-beta.1` — Planning Gatekeeper (§6.1) + Execution Evidence Navigator (§6.2) + the Logic Backbone DESIGN gate.
...
| 覆盖率 | 满足发布门 |
| npm | `latest` → `0.3.0-beta.1`；`beta` → `0.3.0-beta.1`；`alpha` → `0.1.0-alpha.2`（历史保留） |
...
- **npm 发布纪律**：SOP 0.3.0 主干以 `o-coding-navigation@0.3.0-beta.1` 形式发布，遵循严格的预发布清单与 `prepublishOnly` 门，`files` allowlist 收敛到 `dist/` + LICENSE + README + `docs/quickstart.md` + `docs/mcp-usage.md`；`latest` 与 `beta` 都指向 `0.3.0-beta.1`，`alpha` 仍保留在 `0.1.0-alpha.2`；`v0.3.0-beta.1` 为带注释的 git tag + GitHub pre-release。
...
从 v0.3.0-beta.1 开始，npm latest 与 beta 均指向 SOP 0.3.0（逻辑主干）版本。
...
| 渠道 | 版本 | npm tag | 说明 |
|---|---|---|---|
| `latest`（推荐） | `0.3.0-beta.1` | `latest` | SOP 0.3.0（逻辑主干）主干。 |
| Beta（显式预发布通道） | `0.3.0-beta.1` | `beta` | 与 `latest` 同一份产物；想明确固定在预发布通道时使用 `@beta`。 |
...
- 当前外部包：`0.3.0-beta.1`——Planning Gatekeeper（§F.1）+ Execution Evidence Navigator（§F.2）+ Logic Backbone DESIGN 门禁。
