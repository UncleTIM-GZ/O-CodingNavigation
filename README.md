...
> **Phase**: SOP 0.5.0 (Task Backbone) mainline · **Status**: pre-GA beta · **Public**: on npm as `latest` and `@beta` → [`0.5.0-beta.1`](https://www.npmjs.com/package/o-coding-navigation) · GitHub pre-release: [`v0.5.0-beta.1`](https://github.com/UncleTIM-GZ/O-CodingNavigation/releases/tag/v0.5.0-beta.1)
...
| npm | `latest` → `0.5.0-beta.1`; `beta` → `0.5.0-beta.1`; `alpha` → `0.1.0-alpha.2` (historical; preserved) |
...
- **npm publish discipline**: SOP 0.5.0 mainline published as `o-coding-navigation@0.5.0-beta.1` under strict pre-publish checklists, `prepublishOnly` gate, and `files` allowlist. `latest` and `beta` both point to `0.5.0-beta.1`; `alpha` is preserved at `0.1.0-alpha.2` for historical use. Annotated git tag `v0.5.0-beta.1` and matching GitHub pre-release published.
...
As of v0.5.0-beta.1, npm latest and beta both point to the SOP 0.5.0 (Task Backbone) release.

Verify:

```bash
ocn --version       # 0.5.0-beta.1
ocn --help
...
|---|---|---|---|
| `latest` (recommended) | `0.5.0-beta.1` | `latest` | SOP 0.5.0 (Task Backbone) mainline. |
| Beta (explicit prerelease pin) | `0.5.0-beta.1` | `beta` | Same artifact as `latest`; use `@beta` when you want to pin the prerelease channel explicitly. |
...
| `ocn rewind --to <stepId> --reason <text> [--json]` | Move the cursor back to a strictly-earlier step in this round (DEC-033 / AM-008). `--reason` is mandatory; docs/ artifacts are untouched; no gate exemption — every advance after a rewind re-runs the full gate stack. Human-only — never exposed over MCP. | Writes `state.json` (atomic, lock-protected; `latestGateResult` cleared). | `cursor_rewind` (success and failed) |
| `ocn cycle new --yes [--json]` | Archive this round's runtime state to `.ocoding/cycles/<n>-<ts>/` and restart at the first step (DEC-033 / AM-008). docs/ are kept so next-round gates fast-forward; the pin and user-owned `config.yaml` stay; the audit JSONL is never archived — one continuous log spans all rounds. `--yes` is mandatory. Human-only — never exposed over MCP. | Moves `.ocoding` runtime files into the archive; rewrites snapshots + `state.json`. | `cycle_started` (success and failed) |
...
- Current external package: `0.5.0-beta.1` — Planning Gatekeeper (§6.1) + Execution Evidence Navigator (§6.2) + the Logic Backbone DESIGN gate + the Readiness Backbone cross-cutting gate + the Task Backbone BUILD task ledger.
...
| 覆盖率 | 满足发布门 |
| npm | `latest` → `0.5.0-beta.1`；`beta` → `0.5.0-beta.1`；`alpha` → `0.1.0-alpha.2`（历史保留） |
...
- **npm 发布纪律**：SOP 0.5.0 主干以 `o-coding-navigation@0.5.0-beta.1` 形式发布，遵循严格的预发布清单与 `prepublishOnly` 门，`files` allowlist 收敛到 `dist/` + LICENSE + README + `docs/quickstart.md` + `docs/mcp-usage.md`；`latest` 与 `beta` 都指向 `0.5.0-beta.1`，`alpha` 仍保留在 `0.1.0-alpha.2`；`v0.5.0-beta.1` 为带注释的 git tag + GitHub pre-release。
...
从 v0.5.0-beta.1 开始，npm latest 与 beta 均指向 SOP 0.5.0（任务主干）版本。
...
| 渠道 | 版本 | npm tag | 说明 |
|---|---|---|---|
| `latest`（推荐） | `0.5.0-beta.1` | `latest` | SOP 0.5.0（任务主干）主干。 |
| Beta（显式预发布通道） | `0.5.0-beta.1` | `beta` | 与 `latest` 同一份产物；想明确固定在预发布通道时使用 `@beta`。 |
...
| `ocn rewind --to <stepId> --reason <text> [--json]` | 把游标拨回本轮中严格更早的一步（DEC-033 / AM-008）。`--reason` 必填；docs/ 产物一律不动；回拨零豁免——之后每次 advance 重过完整门禁。人类专属——不暴露 MCP。 | 写 `state.json`（原子、带锁；`latestGateResult` 置空）。 | `cursor_rewind`（成功与失败） |
| `ocn cycle new --yes [--json]` | 把本轮运行时状态归档至 `.ocoding/cycles/<n>-<ts>/` 并从首步重开新一轮（DEC-033 / AM-008）。docs/ 保留供下一轮门禁快进；pin 与用户所有的 `config.yaml` 不变；审计 JSONL 不随轮归档——单链贯穿所有轮次。`--yes` 强制。人类专属——不暴露 MCP。 | 运行时文件移入归档；重写快照 + `state.json`。 | `cycle_started`（成功与失败） |
...
- 当前外部包：`0.5.0-beta.1`——Planning Gatekeeper（§F.1）+ Execution Evidence Navigator（§F.2）+ Logic Backbone DESIGN 门禁 + Readiness Backbone 横切门禁 + Task Backbone BUILD 任务台账。
