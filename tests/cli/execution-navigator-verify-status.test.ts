...
describe("ocn verify status — verification status summarizer (DEC-024 MVP 5)", () => {
  let dir: string;

  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "ocn-verify-cli-"));
  });
  ...
  it("text mode prints 'Verification status:' line", async () => {
    writePackageJson(dir);
    writeAcceptance(dir, "## Acceptance Criteria\n\n- AC-001 init\n");
    const result = await spawnOcn(["verify", "status"], { cwd: dir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toMatch(/Verification status:/);
    expect(result.stdout).toMatch(/Ready for review:/);
  }, 30_000);

  it("--json returns envelope with implemented:true", async () => {
    writePackageJson(dir);
    writeAcceptance(dir, "## Acceptance Criteria\n\n- AC-001 init\n");
    const result = await spawnOcn(["verify", "status", "--json"], { cwd: dir });
    expect(result.exitCode).toBe(0);
    ...
    expect(typeof parsed.data.verification.status).toBe("string");
  }, 30_000);

  it("--mode local --json excludes pr and github from sources", async () => {
    writePackageJson(dir);
    writeAcceptance(dir, "## Acceptance Criteria\n\n- AC-001 init\n");
    const result = await spawnOcn(["verify", "status", "--mode", "local", "--json"], {
      cwd: dir,
    });
    expect(result.exitCode).toBe(0);
    const parsed = JSON.parse(result.stdout);
  ...
  it("--mode combined --pr 66 --json includes github via fixture runner", async () => {
    writePackageJson(dir);
    writeAcceptance(dir, "## Acceptance Criteria\n\n- AC-001 init\n");
    const prJson = JSON.stringify({
      ...
      isDraft: false,
      url: "https://github.com/x/y/pull/66",
    ...
    const fixturePath = writeFixture(dir, [
      ...
      { args: ["pr", "view", "66", "--json", PR_VIEW_FIELDS], ok: true, stdout: prJson, stderr: "" },
    ]);
    const result = await spawnOcn(
      ["verify", "status", "--mode", "combined", "--pr", "66", "--json"],
    ...
    expect(parsed.data.pr).not.toBeNull();
    expect(parsed.data.pr.number).toBe(66);
  }, 30_000);

  it("invalid --mode foo returns ERR_ARTIFACT_INVALID exit 2", async () => {
    const result = await spawnOcn(["verify", "status", "--mode", "foo", "--json"], {
      cwd: dir,
    });
  ...
  it("invalid --pr abc returns ERR_ARTIFACT_INVALID and runner mock not called", async () => {
    ...
    const result = await spawnOcn(["verify", "status", "--pr", "abc", "--json"], {
      cwd: dir,
  ...
  it("--mode pr without --pr returns ERR_ARTIFACT_INVALID exit 2", async () => {
    const result = await spawnOcn(["verify", "status", "--mode", "pr", "--json"], {
      cwd: dir,
    });
    expect(result.exitCode).toBe(2);
    const parsed = JSON.parse(result.stdout);
  ...
  it("does not create .ocoding/execution", async () => {
    await spawnOcn(["verify", "status", "--json"], { cwd: dir });
