...
describe("ocn — SOP 0.3.0 full 20-step flow (CLI)", () => {
  ...
  // SOP 0.7.0 (DEC-039) — fresh default init now pins 0.7.0 and snapshots
  // the readiness rulebook alongside the classic snapshot files.
  it("fresh `ocn init` writes 0.7.0 state.json + snapshot files", async () => {
    ...
    expect(configYaml).toContain("0.7.0");
