...
const waiver = (dec: string): OutcomeWaiver => ({
  dec,
  reason: "r",
  at: "2026-07-02T00:00:00.000Z",
});
const DEC_LOG = "# Decision Log\n\n### DEC-043 Outcome Backbone\n- **DEC-041** Acceptance\n";

describe("dec-log parser", () => {
  it("normalises leading zeros (DEC-043 ≡ DEC-43)", () => {
    expect(normaliseDecId("DEC-043")).toBe("DEC-43");
    expect(normaliseDecId("dec-43")).toBe("DEC-43");
  ...
  it("collects declared ids from headings and list items, not prose", () => {
    ...
    expect(ids.has("DEC-43")).toBe(true);
  ...
  it("decExists matches normalised ids", () => {
    expect(decExists(DEC_LOG, "DEC-43")).toBe(true);
    expect(decExists(DEC_LOG, "DEC-043")).toBe(true);
...
describe("evaluateOutcomeSpecGate", () => {
  it("passes when >=1 outcome AC is declared", () => {
    const r = evaluateOutcomeSpecGate({
      specs: [build("AC-1"), outcome("AC-2")],
      noOutcomeWaiver: undefined,
      decLogContent: "",
    });
  ...
  it("blocks when the no-outcome waiver cites a missing DEC", () => {
    const r = evaluateOutcomeSpecGate({
      specs: [build("AC-1")],
      noOutcomeWaiver: waiver("DEC-777"),
      decLogContent: DEC_LOG,
    });
    expect(r.ok).toBe(false);
  ...
  it("passes with a valid DEC-backed no-outcome waiver", () => {
    const r = evaluateOutcomeSpecGate({
      specs: [build("AC-1")],
      noOutcomeWaiver: waiver("DEC-043"),
      decLogContent: DEC_LOG,
    });
    expect(r.ok).toBe(true);
  });
