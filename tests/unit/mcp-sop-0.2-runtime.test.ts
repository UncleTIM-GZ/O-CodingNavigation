...
describe("MCP runtime under 0.5.0 cutover (DEC-032)", () => {
  ...
  it("navigator.where_am_i reports sopProfileVersion 0.9.0 for a fresh project (AM-017)", async () => {
    const result = await whereAmI.handler({ projectRoot: project.cwd });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.data?.project.sopProfileVersion).toBe("0.9.0");
  ...
  it("navigator.brief reflects the 0.9.0 current step + artifact (AM-017)", async () => {
  ...
  it("navigator.detect_sop_version reports 0.9.0 / 0.9.0 with no diff (AM-017)", async () => {
    const result = await detectSopVersionTool.handler({ projectRoot: project.cwd });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.data?.lockedSopProfileVersion).toBe("0.9.0");
      expect(result.data?.currentOcnSopProfileVersion).toBe("0.9.0");
