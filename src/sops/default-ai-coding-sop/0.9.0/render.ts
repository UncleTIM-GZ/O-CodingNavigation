...
// SOP 0.9.0 — renders the canonical 0.9.0 profile data into the YAML snapshot
// strings persisted under .ocoding/. Emission is identical to 0.8.0 (stable
// line ordering, diffable against fixtures); state_ship / state_reflect now
// carry one step each (step_release / step_evolution_report), so the SHIP and
// REFLECT states stop emitting `steps: []`. Every field derives from data.ts so
// the snapshot can never drift from the runtime profile.
